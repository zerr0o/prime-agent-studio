...
### Modèles avancés et budgets autonomes (Prime Agent 0.9.5)

La carte **Modèles avancés**, dans le même écran **Configurer**, expose les nouveaux réglages natifs de Prime Agent 0.9.5 sans changer vos valeurs actuelles. Tant que les champs restent vides, le comportement natif est préservé : aucun modèle de secours ni mode autonome n’est activé automatiquement, et les modèles par défaut restent inchangés.

Le **modèle d’affinage** (`auxiliaryModel`) sert à l’auto-affinage quand il est défini et authentifié ; sinon, le modèle de la session est utilisé. Le **modèle de secours** (`providerBackupModel`) reste désactivé par défaut : les demandes ne changent jamais de modèle en silence. Quand il est défini et authentifié, il ne sert qu’en cas de quota atteint ou de fournisseur indisponible. Les deux sélecteurs utilisent le catalogue authentifié (`/api/models`, modèles `unavailable` exclus) ; un choix vide supprime le champ et rétablit le comportement natif.
