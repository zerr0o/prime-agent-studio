...
Les liens ne sont plus limités à certaines extensions : un fichier binaire, comme un `.exe` ou un `.zip`, peut ouvrir le panneau sans être exécuté. Le menu contextuel d’un lien propose **Ouvrir**, **Ouvrir le dossier** et **Copier le chemin**. Le panneau comporte aussi **Ouvrir le dossier**, à côté d’**Ouvrir**, pour ouvrir le dossier contenant le fichier sur le PC hôte.
...
L’accès reste limité aux projets enregistrés et au code d’accès distant habituel. Les dossiers techniques (`.git`, `node_modules`, caches), les données privées du moteur et du Studio et les liens sortant du projet sont exclus. Un lien explicite vers `.local` est accessible uniquement si ce dossier ne contient pas les données privées du Studio. Le mode distant en lecture seule autorise ces consultations sans autoriser de commandes sur l’agent.
