...
/** Read-only probe for an explicit interpreter. True when fixed, false when
 *  the supported unpatched form is found. Throws on unrecognized form. */
export async function checkKernelCompat(
  python,
  { run, env = process.env, signal, compatScript = KERNEL_COMPAT_SCRIPT } = {},
) {
  if (typeof run !== 'function') throw new Error('kernel compat: execute runner required');
  try {
    await run(
      python,
      [compatScript, '--check-only', '--json'],
    ...
    if (/KERNEL_COMPAT_UNPATCHED/.test(error?.message || '')) return false;
    throw error;
  }
}

/** Actionable rejection for an explicit override that lacks the fix. */
export function externalCompatError(python) {
  return new Error(
    'Python externe sans le correctif noyau PR2372 : ' +
      python +
      '. External Python lacks the PR2372 kernel fix (synchronous bash.consumed). ' +
      'Unset PRIME_AGENT_KERNEL_PYTHON to use the managed patched kernel, or validate that interpreter with ' +
      python +
      ' ' +
      KERNEL_COMPAT_SCRIPT +
      ' --check-only (exit 0 means fixed). Explicit overrides are never modified in place.',
  );
}
