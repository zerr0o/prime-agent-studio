...
async function writeDocument(project, document) {
  ...
  try {
    ...
    await handle.close();
    handle = null;
    // Windows can transiently refuse this atomic rename under parallel load.
    // Retry briefly, keeping the lock and rechecking paths after each wait.
    // Persistent failures still report roadmap_write_failed below.
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        if (process.platform !== 'win32' || !['EPERM', 'EACCES'].includes(reason?.code) || attempt >= 4)
          throw reason;
        await delay(25);
      }
    }
