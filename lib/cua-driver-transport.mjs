...
export function createCuaDriverTransport(options = {}) {
  ...
  async function spawnProxy() {
    ...
    proxy = spawnProcess(driverPath, mcpArgs, {
      ...
      shell: false,
      windowsHide: true,
      // Win32 detach skips the extra libuv child Job so the guardian can
      // nest-adopt this same handle into the CUA Job (else win32 5).
      // No breakaway flag: Studio job inheritance, stdio, pid and kill stay.
