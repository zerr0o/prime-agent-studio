...
function selection(document, targets, instructions = '') {
  ...
  for (const target of targets) {
    ...
    if (target.kind === 'plan') {
      ...
      if (plan.archived)
        throw fail(404, 'roadmap_missing', 'Ce plan est archivé. Restaurez-le avant de lancer un travail.');
      ...
      const plans = document.plans.filter(
        (plan) => plan.milestone === milestone.id && plan.status !== 'abandoned' && !plan.archived,
      );
