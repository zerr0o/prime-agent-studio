 ...
 *     arm_external_input, disarm_external_input, cleanup_external_input,
 *     job_launch, job_adopt, job_status, job_kill, job_reconcile)
...
export const COMPUTER_USE_METHODS = [
  ...
  'stop',
  'arm_external_input',
  'disarm_external_input',
  'cleanup_external_input',
  'job_launch',
  'job_adopt',
  'job_status',
  'job_kill',
...
function validateCommand(command) {
  ...
  switch (method) {
    ...
    case 'arm_external_input': {
      // Adapter held-input plan: metadata only, arming never presses. The
      // worker admits buttons left/right/middle (at most 12) and keys by
      // name (at most 8, resolved by its existing key parser).
      const { buttons = [], keys = [] } = resolved;
      if (!Array.isArray(buttons) || buttons.length > 12)
        throw driverError('INVALID_PARAMS', 'arm_external_input.buttons must list at most 12 buttons.');
      for (const button of buttons) {
        if (typeof button !== 'string' || !['left', 'right', 'middle'].includes(button.trim().toLowerCase()))
          throw driverError('INVALID_PARAMS', 'arm_external_input.buttons must be left, right or middle.');
      }
      if (!Array.isArray(keys) || keys.length > 8)
        throw driverError('INVALID_PARAMS', 'arm_external_input.keys must list at most 8 keys.');
      for (const key of keys) {
        if (typeof key !== 'string' || !key.trim() || key.length > 32)
          throw driverError('INVALID_PARAMS', 'arm_external_input.keys must be non-empty key names.');
      }
      if (buttons.length === 0 && keys.length === 0)
        throw driverError('INVALID_PARAMS', 'arm_external_input needs at least one button or key.');
      break;
    }
    case 'disarm_external_input':
    case 'cleanup_external_input':
      break;
    case 'job_launch': {
      // Mandatory owned-process supervision: suspended launch into a
      // KILL_ON_JOB_CLOSE job with an isolated child env. The worker admits
      // the private nonce, requires the desktop mutex, and fails closed.
      const { exe, args = [], env, nonce } = resolved;
      if (typeof exe !== 'string' || !exe || exe.length > 512)
        throw driverError('INVALID_PARAMS', 'job_launch.exe must be a non-empty path.');
      if (!Array.isArray(args) || args.length > 32)
        throw driverError('INVALID_PARAMS', 'job_launch.args must list at most 32 arguments.');
      for (const arg of args) {
        if (typeof arg !== 'string' || arg.length > 2048)
          throw driverError('INVALID_PARAMS', 'job_launch.args entries must be short strings.');
      }
      if (typeof env !== 'object' || env === null || Array.isArray(env))
        throw driverError('INVALID_PARAMS', 'job_launch.env must be an object.');
      if (typeof nonce !== 'string' || !/^[A-Za-z0-9_-]{16,64}$/.test(nonce))
        throw driverError('INVALID_PARAMS', 'job_launch.nonce must match [A-Za-z0-9_-]{16,64}.');
      break;
    }
    case 'job_adopt': {
      // Opened-handle adoption only: the worker verifies exact image, real
      // Node parent, birth boundary and private socket nonce on the handle.
      // A caller parentPid is a cross-checked claim only, never trust.
      const { pid, exe, parentPid, birthMs, socketNonce } = resolved;
      if (!isInt(pid) || pid <= 0)
        throw driverError('INVALID_PARAMS', 'job_adopt.pid must be a positive integer.');
      if (typeof exe !== 'string' || !exe || exe.length > 512)
        throw driverError('INVALID_PARAMS', 'job_adopt.exe must be a non-empty path.');
      if (parentPid !== undefined && (!isInt(parentPid) || parentPid <= 0))
        throw driverError('INVALID_PARAMS', 'job_adopt.parentPid must be a positive integer.');
      if (typeof birthMs !== 'number' || !(birthMs >= 0))
        throw driverError('INVALID_PARAMS', 'job_adopt.birthMs must be a number.');
      if (typeof socketNonce !== 'string' || !socketNonce || socketNonce.length > 512)
        throw driverError('INVALID_PARAMS', 'job_adopt.socketNonce must be a short non-empty string.');
      break;
    }
    case 'job_status':
      break;
    case 'job_kill': {
      if (
        ...
        (!isInt(resolved.timeoutMs) || resolved.timeoutMs < 1 || resolved.timeoutMs > 5000)
      )
        throw driverError('INVALID_PARAMS', 'job_kill.timeoutMs must be in [1, 5000].');
      break;
    }
    case 'job_reconcile': {
      if (typeof resolved.nonce !== 'string' || !/^[A-Za-z0-9_-]{16,64}$/.test(resolved.nonce))
        throw driverError('INVALID_PARAMS', 'job_reconcile.nonce must match [A-Za-z0-9_-]{16,64}.');
      break;
    }
  }
...
// (status, windows list, observe, the external input supervision
// bookkeeping, and the owned-process job supervision) is read-only and never
// auto-stops. Cleanup and job kill only release what they own and stay
// accepted while stopped; the worker enforces mutex, generation and latch
// gates for launch and adopt.
