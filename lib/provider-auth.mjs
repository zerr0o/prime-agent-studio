...
const guidance = {
  ...
  // Muse Code subscription guidance (experimental warning, OAuth only).
  [MUSE_CODE_PROVIDER_ID]: tr('providers.muse_code_guidance'),
};
export async function createProviderAuth({ agentHome, native } = {}) {
  ...
  // protocol adapter is owned by lib/muse-oauth-provider.mjs; when it is
  // missing or invalid the provider stays invisible: no stub registration,
  // no API key form, no environment fallback.
  let museCodeAvailable = false;
  let museCanonical = false;
  try {
    const loaded = await loadMuseCodeConfig(() => import('./muse-oauth-provider.mjs'));
    if (loaded) {
      museCanonical = ensureMuseCodeProvider(registry, loaded.config, {
        hasUserConfig: hasUserMuseCodeConfig(join(agentHome, 'models.json')),
      });
      museCodeAvailable = museCanonical;
    }
  } catch {
    museCodeAvailable = false;
    museCanonical = false;
  }
  ...
  function item(id) {
    ...
    // Muse Code is OAuth-only, but only when its real adapter is present. Without
    // the adapter the id stays invisible (or behaves as a generic custom entry
    // when the user defined it by hand in models.json).
    if (
      !['amazon-bedrock', 'google-vertex', 'openai-codex', 'github-copilot'].includes(id) &&
      !(museCodeAvailable && id === MUSE_CODE_PROVIDER_ID)
    )
    ...
    const placeholder =
      ...
      (id === MUSE_CODE_PROVIDER_ID &&
        isMuseCodePlaceholderStatus({
          canonical: museCanonical,
          source: status.source,
          storedIsOAuth: isMuseCodeStoredOAuth(stored),
        }));
    return {
      id,
